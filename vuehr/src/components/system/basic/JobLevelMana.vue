...
<script>
    export default {
        ...
        methods:{
            ...
            doUpdate(){
                this.putRequest("/system/basic/jobLevel/jobLevel",this.updateJl).then(resp=>{
                    if(resp){
                        this.initJobLevel();
            ...
            deleteHandler(data) {
                this.$confirm('此操作将永久删除【'+data.name+'】职称, 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/system/basic/jobLevel/jobLevel/"+data.id).then(resp=>{
                        if(resp){
                            this.initJobLevel();
                        }
                    })
            ...
            deleteMany(){
                this.$confirm('此操作将永久删除【'+this.multipleSelection.length+'】条记录, 是否继续?', '提示', {
                    ...
                    this.deleteRequest("/system/basic/jobLevel/jobLevel/"+ids).then(resp=>{
                        if(resp){
                            this.initJobLevel();
                        }
                    })
